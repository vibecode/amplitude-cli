...
export function registerChartCommands(program: Command): void {
  ...
  charts
    .command("create")
    .description(
      "Create a chart from a JSON definition (reads from stdin or --definition)"
    )
      ...
      try {
        ...
        if (opts.save && opts.name) {
          ...
          if (editId) {
            ...
            const saveResult = await mcp.saveChart(
              editId,
              opts.name,
              opts.description
            );
            ...
            console.error(
              "Warning: Could not extract editId from result. Chart not saved."
            );
  ...
  charts
    ...
    .option(
      "--type <types>",
      "Entity types (comma-separated): EVENT, CUSTOM_EVENT, COHORT, DASHBOARD",
      "EVENT,CUSTOM_EVENT"
    )
 ...
 */
function extractMcpText(result: { content: Array<{ type: string; text?: string }> }): unknown {
 ...
 */
function extractEditId(data: unknown): string | null {
