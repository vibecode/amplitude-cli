 ...
 */
function buildEventParam(
  eventType: string,
  groupBy?: string,
  filters?: string
): string {
  const event: Record<string, unknown> = { event_type: eventType };
  ...
  if (filters) {
    try {
      ...
      console.error(
        'Warning: Could not parse --filters as JSON. Expected format: [{"subprop_type":"event","subprop_key":"prop","subprop_op":"is","subprop_value":["val"]}]'
      );
    }
  }
...
export function registerQueryCommands(program: Command): void {
  ...
  query
    .command("segment")
    .description(
      "Event segmentation — count events/users over time with filters and group-by"
    )
    ...
    .option(
      "-m, --metric <metric>",
      "Metric: uniques, totals, avg, pctdau, formula",
      "uniques"
    )
    .option(
      "-i, --interval <n>",
      "Interval: 1 (daily), 7 (weekly), 30 (monthly), -300000 (realtime)",
      "1"
    )
  ...
  query
    ...
    .requiredOption(
      "-e, --events <types...>",
      "Event types in funnel order (space-separated)"
    )
    .requiredOption("--from <date>", "Start date (YYYY-MM-DD or YYYYMMDD)")
    .requiredOption("--to <date>", "End date (YYYY-MM-DD or YYYYMMDD)")
  ...
  query
    ...
    .requiredOption(
      "--start-event <type>",
      "Starting event (e.g. signup, _new)"
    )
    .requiredOption(
      "--return-event <type>",
      "Return event (e.g. _active, purchase)"
    )
    .requiredOption("--from <date>", "Start date (YYYY-MM-DD or YYYYMMDD)")
    .requiredOption("--to <date>", "End date (YYYY-MM-DD or YYYYMMDD)")
    .option(
      "--retention-type <type>",
      "Type: bracket, unbounded, n-day (omit for standard retention)"
    )
    .option("--segment <json>", "Segment definition as JSON array")
    .option("-f, --format <format>", "Output format: json, compact, csv", "json")
    .action(async (opts) => {
  ...
  query
    ...
    .option(
      "-m, --metric <type>",
      "Metric: total, paying, arppu, arpu, avg-revenue, ltv",
      "total"
    )
    .option(
      "-i, --interval <n>",
