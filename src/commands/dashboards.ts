 ...
 */
function buildRowsFromChartIds(
  chartIds: string[],
  cols: number = 2
...
export function registerDashboardCommands(program: Command): void {
  ...
  dashboards
    ...
    .option(
      "--charts <ids...>",
      "List of saved chart IDs to include in the dashboard"
    )
    .option(
      "--cols <n>",
      "Charts per row when using --charts (1-4, default 2)",
      "2"
    )
    .option("-f, --format <format>", "Output format: json, compact, csv", "json")
    .action(async (opts) => {
      try {
        ...
        if (opts.charts && opts.charts.length > 0) {
          ...
          rows = buildRowsFromChartIds(opts.charts as string[], cols);
